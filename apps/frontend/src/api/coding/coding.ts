...
/**
 * Sends a message to a coding session. The agent processes it in the background.
...
export async function sendMessage(
  ...
  if (!res.ok) {
    const body = await res.text();
    throw new Error(
      `Coding request failed (${res.status.toString()}): ${body}`,
    );
  }
}

/**
 * Subscribes to SSE events from a coding session.
